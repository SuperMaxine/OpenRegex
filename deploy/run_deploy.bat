...
docker-compose -f "%PROJECT_ROOT%\deploy\docker-compose.yml" --env-file "%PROJECT_ROOT%\.env" down -v

echo.
echo Starting a fresh development environment...
docker-compose -f "%PROJECT_ROOT%\deploy\docker-compose.yml" --env-file "%PROJECT_ROOT%\.env" up --build -d
